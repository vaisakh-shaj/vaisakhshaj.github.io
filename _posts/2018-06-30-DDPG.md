...
This is achieved by using an actor neural network $\mu_\theta(s)$, and a deterministic policy $\mu_{\theta}(s)$
 and update the policy parameters by moving them in the direction of the gradient of the action-value function.
